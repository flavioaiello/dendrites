...
text = re.sub(r'(\s*let events: Vec<DomainEvent> = evts)', r'''
...
text = re.sub(r'(for svc in &bc\.services \{)', r'''
...
text = re.sub(r'(\s*let _ = self\.db\.run_script\(\n\s*"\?\[workspace, context, name, state\] := \*service\{)', r'''
        let _ = self.db.run_script(
